...
class Lambda(Node):
    ...
    def __init__(self, args=None, body=None, node_type=NodeType.LAMBDA):
        if args is None:
            args = []
        super().__init__(node_type=node_type)
        self.args = args
...
class ListComp(Node):
    ...
    generators: List[Dict[str, Node]] = field(default_factory=list)

    def __init__(self, elt=None, generators=None, node_type=NodeType.LIST_COMP):
...
@dataclass
class DictComp(Node):
    """Dictionary comprehension node."""
    key: Optional[Node] = None
    value: Optional[Node] = None
    generators: List[Dict[str, Node]] = field(default_factory=list)

    def __init__(self, key=None, value=None, generators=None, node_type=NodeType.DICT_COMP):
        if generators is None:
            generators = []
        super().__init__(node_type=node_type)
        self.key = key
...
class SetComp(Node):
    ...
    def __init__(self, elt=None, generators=None, node_type=NodeType.SET_COMP):
        if generators is None:
            generators = []
        super().__init__(node_type=node_type)
        self.elt = elt
...
class GeneratorExp(Node):
    ...
    def __init__(self, elt=None, generators=None, node_type=NodeType.GENERATOR_EXP):
        if generators is None:
            generators = []
        super().__init__(node_type=node_type)
        self.elt = elt
...
@dataclass
class Global(Node):
    """Global statement node."""
    names: List[str] = field(default_factory=list)

    def __init__(self, names=None, node_type=NodeType.GLOBAL):
        if names is None:
            names = []
        super().__init__(node_type=node_type)
        self.names = names
...
@dataclass
class Nonlocal(Node):
    """Nonlocal statement node."""
    names: List[str] = field(default_factory=list)

    def __init__(self, names=None, node_type=NodeType.NONLOCAL):
        if names is None:
            names = []
        super().__init__(node_type=node_type)
        self.names = names
...
NODE_CLASSES = {
    ...
    NodeType.LIST_COMP: ListComp,
    NodeType.DICT_COMP: DictComp,
    NodeType.SET_COMP: SetComp,
    NodeType.GENERATOR_EXP: GeneratorExp,
    NodeType.GLOBAL: Global,
    NodeType.NONLOCAL: Nonlocal,
